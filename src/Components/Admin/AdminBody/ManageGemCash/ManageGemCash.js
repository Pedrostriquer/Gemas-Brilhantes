// Dentro de src/Components/Admin/AdminBody/ManageGemCash/ManageGemCash.js

import React, { useState, useEffect } from 'react';
import { doc, getDoc, setDoc } from 'firebase/firestore';
import { db } from '../../../../firebase/config';
import './ManageGemCash.css';
...
const ManageGemCash = () => {
    ...
    const handleChange = (section, field, value, index = null, pointType = null) => {
        setGemCashData(prevData => {
            // --- A CORREÇÃO ESTÁ AQUI: Criamos uma cópia profunda do estado ---
            ...
            if (section === 'problemSolution' && pointType) {
                ...
                // Agora, esta linha é segura, pois 'newData[section]' sempre existirá
                newData[section][field] = value;
            }
            return newData; // Retorna a nova cópia, não o estado mutado
        });
    };
                 ...
                 <div className="ps-editor-grid">
                    <div className="ps-column">
                        ...
                        {gemCashData.problemSolution.traditionalPoints.map((point, index) => (
                            <div className="ps-point-editor" key={index}>
                ...
                <div className="hiw-details-grid">
                    {gemCashData.howItWorks.details.map((item, index) => (
                        <div className="hiw-detail-editor" key={item.id}>
                            <h4 className="item-title-input">{item.title}</h4>
                            <div className="form-group">
